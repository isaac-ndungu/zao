    ...
    <div>
      ...
      <div className="bg-surface-container rounded-xl border border-outline-variant p-4 mb-4">
        ...
        <form onSubmit={handlePasswordChange} className="space-y-3">
          <div><label htmlFor="id-current-password" className="block text-xs font-semibold text-on-surface-variant mb-1.5">{t('currentPassword')}</label><input id="id-current-password" value={pwForm.old_password} onChange={(e) => setPwForm(p => ({ ...p, old_password: e.target.value }))} type="password" required className="w-full px-3.5 py-3 rounded-xl border-2 border-outline-variant bg-surface text-sm outline-none focus:border-primary min-h-[44px]" /></div>
          <div><label htmlFor="id-new-password" className="block text-xs font-semibold text-on-surface-variant mb-1.5">{t('newPassword')}</label><input id="id-new-password" value={pwForm.new_password} onChange={(e) => setPwForm(p => ({ ...p, new_password: e.target.value }))} type="password" required minLength={8} className="w-full px-3.5 py-3 rounded-xl border-2 border-outline-variant bg-surface text-sm outline-none focus:border-primary min-h-[44px]" /></div>
          <div><label htmlFor="id-confirm-password" className="block text-xs font-semibold text-on-surface-variant mb-1.5">{t('confirmPassword')}</label><input id="id-confirm-password" value={pwForm.confirm_password} onChange={(e) => setPwForm(p => ({ ...p, confirm_password: e.target.value }))} type="password" required className="w-full px-3.5 py-3 rounded-xl border-2 border-outline-variant bg-surface text-sm outline-none focus:border-primary min-h-[44px]" /></div>
