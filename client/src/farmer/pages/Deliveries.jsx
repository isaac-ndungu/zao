...
const statusColors = {
  PENDING: 'bg-warning-container text-warning', GRADED: 'bg-success-container text-success', ACCEPTED: 'bg-success-container text-success',
  REJECTED: 'bg-error-container text-error', PAID: 'bg-primary-container text-primary',
}

const productIcons = {
  MILK: 'local_drink', COFFEE_CHERRIES: 'coffee', HONEY: 'faucet',
  MACADAMIA: 'nutrition', AVOCADO: 'nutrition',
}
            ...
            <button key={d.id} onClick={() => setSelected(d)} className="bg-surface-container rounded-xl border border-outline-variant p-4 w-full text-left hover:bg-primary-container/10 transition-colors">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl bg-primary-container flex items-center justify-center shrink-0 mt-1">
                  <span className="material-symbols-outlined text-primary text-xl">{productIcons[d.product_type] || 'inventory'}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="font-semibold text-sm text-on-surface truncate">{d.product_type?.replace(/_/g, ' ')}</p>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${statusColors[d.status] || 'bg-gray-200 text-gray-500'}`}>{d.status}</span>
                  </div>
                  <div className="flex items-center gap-3 text-xs text-on-surface-variant">
                    <span>{d.quantity_kg || d.volume_litres ? `${d.quantity_kg || d.volume_litres} ${d.quantity_kg ? 'kg' : 'L'}` : '-'}</span>
                    {d.batch_id && <span>Batch: {d.batch_id}</span>}
          ...
          <div className="bg-surface-container rounded-t-2xl p-6 w-full max-h-[85vh] overflow-y-auto" style={{ animation: 'slideUp 0.3s ease-out' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div><p className="text-xs text-on-surface-variant">{t('productType')}</p><p className="text-sm font-medium">{selected.product_type?.replace(/_/g, ' ')}</p></div>
                <div><p className="text-xs text-on-surface-variant">{t('status')}</p>                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${statusColors[selected.status] || 'bg-gray-200 text-gray-500'}`}>{selected.status}</span></div>
                ...
                {selected.batch_id && <div><p className="text-xs text-on-surface-variant">Batch</p><p className="text-sm font-medium">{selected.batch_id}</p></div>}
              </div>
              {selected.grade && (
                <div className="border-t border-outline-variant pt-3 mt-3">
                  ...
                  {selected.quality_metrics && Object.keys(selected.quality_metrics).length > 0 && (
                    <div className="mt-2">
                      <p className="text-xs text-on-surface-variant mb-1">Quality Metrics</p>
            ...
            <button onClick={() => setSelected(null)} className="bg-primary text-on-primary px-6 py-3 rounded-xl text-sm font-semibold min-h-[44px] hover:opacity-80 w-full mt-6">Close</button>
          </div>
        </div>
      )}
    </div>
