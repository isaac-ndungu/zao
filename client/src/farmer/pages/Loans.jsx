...
const statusColors = {
  PENDING: 'bg-warning-container text-warning', APPROVED: 'bg-info-container text-info', DISBURSED: 'bg-success-container text-success',
  COMPLETED: 'bg-primary-container text-primary', DEFAULTED: 'bg-error-container text-error',
}
...
export default function FarmerLoans() {
  ...
  const handleApply = async (e) => {
    ...
    try {
      ...
      if (!res.ok) { const err = await res.json(); throw new Error(Object.values(err).flat().join(', ') || 'Failed to apply') }
      showToast({ type: 'success', message: 'Loan application submitted.' })
      setShowApply(false)
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-bold">{t('loans') || 'Loans'} <span className="text-sm font-normal text-on-surface-variant">({loans.length})</span></h2>
        <button onClick={() => setShowApply(true)} className="bg-primary text-on-primary px-4 py-2 rounded-xl text-xs font-semibold min-h-[36px] hover:opacity-80">{t('applyLoan')}</button>
      </div>
      ...
      {loans.length === 0 ? (
        <div className="text-center py-12">
          ...
          <button onClick={() => setShowApply(true)} className="bg-primary text-on-primary px-6 py-3 rounded-xl text-sm font-semibold min-h-[44px] hover:opacity-80">{t('applyLoan')}</button>
        </div>
      ) : (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${statusColors[loan.status] || 'bg-gray-200 text-gray-500'}`}>{loan.status}</span>
                  </div>
                  <div className="flex items-center gap-3 text-xs text-on-surface-variant">
                    <span>{loan.interest_rate}%</span>
                    <span>{loan.installments_paid || 0}/{loan.number_of_installments} paid</span>
                  ...
                  {(selected?.id === loan.id) && (
                    <div className="border-t border-outline-variant mt-3 pt-3 space-y-2">
                      {loan.total_repayable && <div className="flex justify-between text-xs"><span>Total Repayable</span><span className="font-bold">{formatKes(loan.total_repayable)}</span></div>}
                      {loan.installment_amount && <div className="flex justify-between text-xs"><span>Installment</span><span className="font-bold">{formatKes(loan.installment_amount)}</span></div>}
                      {loan.guarantors?.length > 0 && (
                        <div>
                          <p className="text-xs text-on-surface-variant mb-1">Guarantors</p>
                          {loan.guarantors.map((g, i) => (
                            <p key={i} className="text-xs">{g.guarantor_name || `Guarantor #${i + 1}`}</p>
                          ))}
                        </div>
                      )}
                      {loan.notes && <p className="text-xs text-on-surface-variant">Notes: {loan.notes}</p>}
                    </div>
                  )}
                </div>
          ...
          <div className="bg-surface-container rounded-t-2xl p-6 w-full max-h-[85vh] overflow-y-auto" style={{ animation: 'slideUp 0.3s ease-out' }} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleApply} className="space-y-4">
              ...
              <div className="flex gap-3">
                <button type="submit" disabled={saving} className="bg-primary text-on-primary px-6 py-3 rounded-xl text-sm font-semibold min-h-[44px] hover:opacity-80 disabled:opacity-40 disabled:cursor-not-allowed flex-1">{saving ? <span className="inline-block animate-spin h-5 w-5 border-2 border-outline-variant border-t-primary rounded-full" /> : t('applyLoan')}</button>
                <button type="button" onClick={() => setShowApply(false)} className="bg-transparent border border-outline-variant px-6 py-3 rounded-xl text-sm font-semibold min-h-[44px] hover:bg-gray-50 flex-1">{t('cancel')}</button>
              </div>
            </form>
          </div>
        </div>
