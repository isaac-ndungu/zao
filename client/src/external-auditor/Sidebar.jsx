...
export default function ExternalAuditorSidebar({ mobileOpen, onClose, minimized }) {
  ...
  const sidebarContent = (
    <aside aria-label="External auditor sidebar" className={`${minimized ? 'w-16 px-2' : 'w-64 px-4'} h-full bg-primary flex flex-col py-6 overflow-y-auto transition-all duration-300`}>
      ...
      <div className={`mt-auto pt-6 border-t border-on-primary/10 ${minimized ? 'space-y-2' : 'space-y-4'}`}>
        {[{ to: '/external-auditor/profile', icon: 'person', label: 'Profile' }].map((item) => (
          <Link
            key={item.to}
            to={item.to}
            onMouseEnter={(e) => showTooltip(e, item.label)}
