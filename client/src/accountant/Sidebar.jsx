...
const navItems = [
  ...
  { to: '/accountant/cycles', icon: 'payments', label: 'Payment Cycles' },
...
const bottomItems = [
  { to: '/accountant/profile', icon: 'person', label: 'Profile' },
  { to: '/accountant/settings', icon: 'settings', label: 'Settings' },
]
    ...
    <aside aria-label="Accountant sidebar" className={`${minimized ? 'w-16 px-2' : 'w-64 px-4'} h-full bg-primary flex flex-col py-6 overflow-y-auto transition-all duration-300`}>
      <div className={`mb-10 ${minimized ? 'px-0 text-center' : 'px-2'}`}>
        {minimized ? (
          ...
          <h1 className="font-headline-lg text-headline-lg font-bold text-on-primary">Zao Accountant</h1>
        )}
      </div>
